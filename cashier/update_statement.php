<?php
  ...
  if(isset($dados) && !empty($dados)){
    ...
    $oldValue = $request->dados->oldValue;
    ...
    try {
      ...
      $updateManager = $conn->prepare("UPDATE statement SET description = :description, value = :value WHERE statement_id = :statement_id");
      ...
      if($user['type']=='admin'&& $userType=='dealer'){
        ...
        cashierDealerUpdatelStatement($value,$oldValue,$type,$user_id,$day,$conn);
        ...
        cashierManagerUpdatelStatement($value,$oldValue,$type,$user_id,$day,$conn);
      }else{
