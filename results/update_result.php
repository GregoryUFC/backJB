<?php
  ...
  if(isset($dados) && !empty($dados)){
    ...
    $result_id = $request->dados->result_id;
    ...
    try {
      ...
      { 
        ...
        while($col = $consulta->fetch(PDO::FETCH_ASSOC))
        {
          ...
          if(floatval($col['reward']) != $reward || $col['reward']===null){
            $update->execute([
                'win'=> $wins==0?0:1,
                'reward'=> $reward,
                'bid_id'=> $col['bid_id']
            ]);
            cashierEditResult($reward,$col['reward'],$row['dealer_id'],$row['time'],$conn);
          }

          $index++;
        }
      }
      $stmt = $conn->prepare("UPDATE results SET prize1= :prize1, prize2= :prize2, prize3= :prize3, prize4= :prize4, prize5= :prize5, prize6= :prize6, prize7= :prize7 WHERE result_id= :result_id");
      $stmt->execute([
        'result_id' => $result_id,
        'prize1' => $prize1,
        'prize2' => $prize2,
